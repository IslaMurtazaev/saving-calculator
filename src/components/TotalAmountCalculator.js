...
const TotalAmountCalculator = () => {
  ...
  const calculateSavingPlan = () => {
    ...
    setSavingPlan([getFormattedNum(monthlyDeposit), monthDiff, getFormattedNum(goal), getFormattedDate(reachBy)]);
  };

  return (
    <>
      <DollarInput label="Total amount" value={goal} setValue={setGoal} />
      <MonthInput label="Reach goal by" date={reachBy} setDate={setReachBy} onlyFuture={true} />
      {savingPlan &&
